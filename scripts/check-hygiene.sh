...
#
# Allow-listed: this script only, which has to contain the patterns it looks for.
...
ALLOWED='^(\./)?scripts/check-hygiene\.sh$'
...
for pattern in "${PATTERNS[@]}"; do
  if grep -riInE --binary-files=without-match "$pattern" "${CANDIDATES[@]}"; then
